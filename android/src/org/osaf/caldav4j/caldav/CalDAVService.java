...
public class CalDAVService extends Service {
    ...
    private void setupAcc (Message msg) {
        ...
        CalDAVAccount acc = new CalDAVAccount (this, calname, username, password, host, protocol, home, coll, port);
        ...
        if (err == 0) {
            ...
            final Account account = new Account(acc.getEmail(), CALDAV_ACC_TYPE);
    ...
    private void sync (Message msg) {
        ...
        int err = 0;
        String errstr = null;
        CalDAVAccount [] accs = Preferences.getPreferences (this).getAccounts ();

        try {
            for (CalDAVAccount a : accs) {
    ...
    private void remove (Message msg) {
        ...
        CalDAVAccount [] accs = prefs.getAccounts ();
    ...
    private int doRemoveCalendar (CalDAVAccount acc) throws Exception {
    ...
    private void doRemoveCalendar (CalDAVAccount acc, long calid) throws Exception {
        mCR.delete (ContentUris.withAppendedId (asSyncAdapter (android.provider.CalendarContract.Calendars.CONTENT_URI, acc.getEmail(), CALDAV_ACC_TYPE), calid), null, null);
    }

    private void doSyncCalendar (CalDAVAccount acc, long calid) throws Exception {
        // Get a list of local events
        String [] evproj1 = new String [] {Events._ID, Events._SYNC_ID, Events.DELETED, Events.DIRTY};
        ...
        Cursor c = mCR.query (Events.CONTENT_URI, evproj1, Events.CALENDAR_ID + "=" + calid, null, null);
        ...
        for (String uid: newdevevs.keySet ())
            addEventOnServer (uid, newdevevs.get (uid), caldavif, asSyncAdapter (Events.CONTENT_URI, acc.getEmail(), CALDAV_ACC_TYPE));
        ...
        for (String uid: removedevs.keySet ()) {
            ...
            removeLocalEvent (removedevs.get (uid), asSyncAdapter (Events.CONTENT_URI, acc.getEmail(), CALDAV_ACC_TYPE));
        }

        //update the dirty events on server
        for (String uid: dirtyevs.keySet ())
            updateEventOnServer (uid, dirtyevs.get (uid), caldavif, asSyncAdapter (Events.CONTENT_URI, acc.getEmail(), CALDAV_ACC_TYPE));
        ...
        for (VEvent v : evs) {
            ...
            if (localevs.containsKey (uid) || dirtyevs.containsKey (uid)) {
                ...
                mCR.update (ContentUris.withAppendedId (asSyncAdapter (Events.CONTENT_URI, acc.getEmail(), CALDAV_ACC_TYPE), tid), cv, null, null);
                ...
                euri = mCR.insert (asSyncAdapter (Events.CONTENT_URI, acc.getEmail(), CALDAV_ACC_TYPE), cv);
        ...
        for (String uid: localevs.keySet ())
            removeLocalEvent (localevs.get (uid), asSyncAdapter (Events.CONTENT_URI, acc.getEmail(), CALDAV_ACC_TYPE));
    ...
    //syncs one calendar
    private int doSyncCalendar (CalDAVAccount acc) throws Exception {
    ...
    private long createCalendar(CalDAVAccount acc) {
        ...
        Uri uri = mCR.insert(asSyncAdapter (Calendars.CONTENT_URI , account, CALDAV_ACC_TYPE), cv);
        if (uri != null) {
    ...
    private long getCalendarId (CalDAVAccount acc) {
        String [] calproj = {Calendars._ID};
        String calacc = acc.getEmail ();
        Cursor c = mCR.query (Calendars.CONTENT_URI, calproj, Calendars.OWNER_ACCOUNT +
    ...
    private void updateEventOnServer (String uid, long id, CalDAV4jIf caldavif, Uri uri) throws Exception {
        ...
        if (ve != null) {
            ...
            ContentValues cv = new ContentValues ();
            cv.put (Events.DIRTY, 0);
            mCR.update (ContentUris.withAppendedId (uri, id), cv, null, null);
        } else
    ...
    private VEvent getEvFromDB (String uid, long id) {
        ...
        Cursor c = mCR.query (ContentUris.withAppendedId (Events.CONTENT_URI, id), proj,null, null, null);
        if (c.moveToFirst ()) {
    ...
    private void addEventOnServer (String uid, long id, CalDAV4jIf caldavif, Uri uri) throws Exception {
        VEvent ve = getEvFromDB (uid, id);
        if (ve != null) {
            caldavif.addEv (ve);
    ...
    private void removeLocalEvent (long id, Uri uri) {
        mCR.delete (ContentUris.withAppendedId (uri, id), null, null);
