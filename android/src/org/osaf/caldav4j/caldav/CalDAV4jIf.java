...
public class CalDAV4jIf extends BaseTestCase {
    ...
    public VEvent [] getEvents () throws Exception {
        ...
        int nowyear = rightNow.get(java.util.Calendar.YEAR);
        int nowmonth = rightNow.get(java.util.Calendar.MONTH);
        int nowdate = rightNow.get(java.util.Calendar.DATE);
        TimeZone curtz = TimeZone.getDefault();
        Date beginDate = ICalendarUtils.createDateTime(nowyear - 1, nowmonth, nowdate, null, true);
        Date endDate = ICalendarUtils.createDateTime(nowyear, nowmonth + 6, nowdate, null, true);
