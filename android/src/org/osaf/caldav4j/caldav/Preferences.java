...
public class Preferences {
     ...
     */
    public CalDAVAccount[] getAccounts() {
        String accountUuids = mSharedPreferences.getString(ACCOUNT_UUIDS, null);
        if (accountUuids == null || accountUuids.length() == 0) {
            return new CalDAVAccount[] {};
        }
        String[] uuids = accountUuids.split(",");
        CalDAVAccount[] accounts = new CalDAVAccount[uuids.length];
        for (int i = 0, length = uuids.length; i < length; i++) {
            accounts[i] = new CalDAVAccount(this, uuids[i]);
        }
        return accounts;
    }
    public boolean accountExists(CalDAVAccount a) {
        String accountUuids = mSharedPreferences.getString(ACCOUNT_UUIDS, null);
        if (accountUuids == null || accountUuids.length() == 0) {
            return false;
        }
        ...
        CalDAVAccount tmp;
        for (int i = 0, length = uuids.length; i < length; i++) {
            tmp = new CalDAVAccount(this, uuids[i]);
            if (tmp.equals (a))
                return true;
        }
        return false;
