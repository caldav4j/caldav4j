...
public class CalDAVCollection extends CalDAVCalendarCollectionBase{
	...
	throws CalDAV4JException {
		...
		// Make the ticket
		MkTicketMethod mkTicketMethod = 
		   methodFactory.createMkTicketMethod(getAbsolutePath(relativePath),ticketRequest);
		try {
	...
	throws CalDAV4JException {
		DelTicketMethod delTicketMethod = 
		   methodFactory.createDelTicketMethod(getAbsolutePath(relativePath),ticketId);
	...
	throws CalDAV4JException, HttpException, IOException {
		...
		//QName ticketDiscoveryProperty = new QName(CalDAVConstants.NS_XYTHOS,
		//		CalDAVConstants.ELEM_TICKETDISCOVERY);
		//QName ownerProperty = new QName(CalDAVConstants.NS_DAV,"owner");
      ...
      PropProperty xythosTag = new PropProperty(CalDAVConstants.NS_XYTHOS,"D",CalDAVConstants.ELEM_TICKETDISCOVERY);
      PropProperty propTag = new PropProperty(CalDAVConstants.NS_DAV,"D","owner");
      propFindTag.addChild(propTag);
      propFindTag.addChild(xythosTag);
		...
		propFindMethod.setPropFindRequest(propFindTag);
      ...
      //			Property item = (Property) responses
      //			.nextElement();
      ...
      List<String> ticketIDList = new ArrayList<String>();
      for (DavProperty<?> prop : propFindMethod.getResponseTable()){
         if (prop.getName().getName().equals(CalDAVConstants.ELEM_TICKETDISCOVERY)){
            ticketIDList.add(""+prop.getValue());
         }
      }
...
*/
					...
					if (response != null) {
						list.add(
						      new CalDAVResource(
						            ...
						            etag,stripHost(response.getHref())));
					}
				}							
			} catch (Exception e) {
	...
	protected List<Calendar> getComponentByMultiget(HttpClient httpClient, String componentName,CalendarMultiget query) throws CalDAV4JException 
	{
		...
		while (e.hasMoreElements()){
			...
			//no checks needed, Calendar already parsed when CaldavStatus.SC_OK)
				CalDAVResource resource = null;
	...
	// manage ACL TODO
	//
	public List<Ace> getAces(HttpClient httpClient) throws CalDAV4JException{
		return getAces(httpClient, null);
	}

	public List<Ace> getAces(HttpClient httpClient, String path) throws CalDAV4JException{
	   PropFindMethod method = null;
	   int status = 0;
	   try {
		   method = methodFactory.createPropFindMethod(
		      getCalendarCollectionRoot() + StringUtils.defaultString(path, ""));	
		   method.setDepth(0);
		
		   PropProperty propFindTag = new PropProperty(CalDAVConstants.NS_DAV,"D", CalDAVConstants.DAV_PROPFIND);
	      PropProperty aclTag = new PropProperty(CalDAVConstants.NS_DAV,"D",CalDAVConstants.DAV_ACL);
	      PropProperty propTag = new PropProperty(CalDAVConstants.NS_DAV,"D",CalDAVConstants.DAV_PROP);
	      propTag.addChild(aclTag);
	      //propTag.addChild(new DisplayName());
	      //propTag.addChild(new CalendarDescription());
	      propFindTag.addChild(propTag);
			method.setPropFindRequest(propFindTag);
	...
	public void setAces(HttpClient client, Ace[] aces, String path) throws CalDAV4JException{
		try {
	      AclMethod method = 
		      new AclMethod(getCalendarCollectionRoot() + StringUtils.defaultString(path, "")
			...
			case CaldavStatus.SC_NOT_FOUND:
				throw new ResourceNotFoundException(ResourceNotFoundException.IdentifierType.PATH, method.getPath());
