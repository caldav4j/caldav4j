 ...
 * @see HttpPut
 */
public class HttpPutMethod<T extends Serializable> extends HttpPut {
     ...
     * @param calendarOutputter Outputter object to generate the calendar. Only required if {@link
     *     CalendarRequest} contains a calendar
     */
    public HttpPutMethod(
            URI uri, ResourceRequest<T> calendarRequest, ResourceParser<T> calendarOutputter) {
        super(uri);
     ...
     */
    public HttpPutMethod(
            String uri, ResourceRequest<T> calendarRequest, ResourceParser<T> calendarOutputter) {
    ...
    protected void generateRequestBody(
            ResourceRequest<T> calendarRequest, ResourceParser<T> calendarOutputter) {
        ...
        if (calendar != null) {
            ...
            try {
                ...
                ContentType ct =
                        ContentType.create(
                                calendarOutputter.getResponseContentType(),
                                calendarRequest.getCharset());
     ...
     * @param calendarRequest Object representing the marshalled properties of the request
     */
    protected void addRequestHeaders(ResourceRequest<T> calendarRequest) {
        boolean ifMatch = calendarRequest.isIfMatch(),
                ifNoneMatch = calendarRequest.isIfNoneMatch();
        if (ifMatch || ifNoneMatch) {
            String name =
                    ifMatch
                            ? CalDAVConstants.HEADER_IF_MATCH
                            : CalDAVConstants.HEADER_IF_NONE_MATCH;
            String value = null;
    ...
    /**
     * Check the provided {@link HttpResponse} for successful execution. This treats all 2xx status
     * codes.
     *
