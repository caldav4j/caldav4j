...
class SocksSocketImplFactory implements SocketImplFactory {
    ...
    public SocketImpl createSocketImpl() {
        try {
            Class<? extends SocketImpl> socketImplClass =
                    Class.forName(SOCKET_IMPL_CLASS_NAME).asSubclass(SocketImpl.class);

            Constructor<? extends SocketImpl> constructor =
                    socketImplClass.getDeclaredConstructor();
            constructor.setAccessible(true);

            return constructor.newInstance();
        } catch (ClassNotFoundException exception) {
            throw new IllegalStateException(
                    "Cannot find SocketImpl class: " + SOCKET_IMPL_CLASS_NAME);
        } catch (NoSuchMethodException exception) {
            throw new IllegalStateException(
                    "Missing default constructor for SocketImpl class: " + SOCKET_IMPL_CLASS_NAME);
        } catch (InstantiationException exception) {
            throw new IllegalStateException(
                    "Error instantiating SocketImpl class: " + SOCKET_IMPL_CLASS_NAME, exception);
        } catch (IllegalAccessException exception) {
            throw new IllegalStateException(
                    "Error instantiating SocketImpl class: " + SOCKET_IMPL_CLASS_NAME, exception);
        } catch (InvocationTargetException exception) {
            throw new IllegalStateException(
                    "Error instantiating SocketImpl class: " + SOCKET_IMPL_CLASS_NAME,
                    exception.getCause());
        }
    }
